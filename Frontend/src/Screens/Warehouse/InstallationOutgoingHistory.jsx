...
//         console.log('Outgoing History Data', response.data.data);
//       } catch (err) {
//         console.log('Show Error', err?.response?.data || err?.message);
...
const InstallationOutgoingHistory = () => {
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.header}>Outgoing Items in Maharastra</Text>
        ...
        outgoingHistory.map((item, index) => (
          <View key={index} style={styles.card}>
            ...
            <View style={styles.cardRow}>
              <Text style={styles.label}>Driver:</Text>
              <Text style={styles.value}>
                {item.driverName || 'N/A'} {item.driverContact ? `(${item.driverContact})` : ''}
              </Text>
            </View>
            <View style={styles.cardRow}>
              <Text style={styles.label}>Status:</Text>
            ...
            {item.itemsList?.length > 0 ? (
              item.itemsList.map((itemDetail, itemIndex) => (
                <View key={itemIndex} style={styles.itemRow}>
                  <Text style={styles.itemName}>
                    {itemDetail.systemItemId?.itemName || 'Unknown Item'}
                  </Text>
                  <Text style={styles.itemQuantity}>
                    Qty: {itemDetail.quantity || '0'}
                  </Text>
                </View>
              ))
            ) : (
              <Text style={styles.noItemsText}>No items in this transfer</Text>
            )}
