...
const WarehouseDashboard = () => {
  ...
  const fetchSystemItems = async (systemId) => {
    try {
      ...
      console.log('Error fetching system items:', error.message);
    }
  };
  ...
  return (
    <ScrollView
              ...
              <View style={styles.dropdownContainer}>
                {systems.map((system) => (
                  <Pressable
                    ...
                    <Text style={styles.dropdownItemText}>{system.systemName}</Text>
      ...
      {activeCard === 'Service' ? (
        ...
        filteredSystemData.length > 0 ? (
          filteredSystemData.map(({ systemItemId, quantity, requiredQuantity, stockLow, materialShort }) => (
            <View key={systemItemId._id} style={[
              styles.card,
              stockLow && styles.lowStockCard
            ]}>
