...
const ApprovalData = () => {
  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      if (response.status === 200 && response.data.pickupItems) {
  ...
  const handleApproveBtn = async (sendTransactionId, incoming) => {
    try {
      const response = await axios.put(`${API_URL}/warehouse-admin/update-incoming-status`, {
        status: true,
  ...
  const renderOrderItem = ({ item }) => (
    ...
    { !(item.status) && <View key={item._id} style={[styles.card, { width: cardWidth }]}>
