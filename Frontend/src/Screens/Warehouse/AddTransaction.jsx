 ...
 useEffect(()=>{
    const selectedItemList = async ()=>{
        ...
        for(let index = 0; index < response.data.items.length; index++){
          updateItemsData.push({_id: index+1, itemName: (response.data.items)[index]})
        }
  ...
  const validateInput = () => {
    if (  !servicePersonName || !servicePerContact || !farmerContact || !farmerName || !farmerVillageName ) {
  ...
  const handleSubmit = async () => {
    ...
    const data = {servicePersonName,servicePerContact,farmerName, farmerContact, farmerVillage: farmerVillageName, items: itemSelected, warehouse: allWarehouses, remark: remarks, serialNumber, incoming: false, pickupDate: new Date() }
    console.log(data);


    try {
      const response = await axios.post(`${API_URL}/warehouse-admin/outgoing-items`,data, 
        {
          headers: {
    ...
    <View style={styles.container}>
      ...
      >
        ...
        <ScrollView contentContainerStyle={{...styles.scrollContainer }}>
          ...
          <TextInput value={servicePersonName} onChangeText={setServicePersonName} placeholder="Enter ServicePerson Name" style={styles.input}   />

          <Text>Service Person Contact:</Text>
          <TextInput value={servicePerContact} onChangeText={setServicePerContact} placeholder="Enter ServicePerson Contact" style={styles.input}   />
