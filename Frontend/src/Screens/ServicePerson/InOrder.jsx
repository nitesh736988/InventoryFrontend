...
const InOrder = ({route}) => {
  ...
  const validateInput = data => {
    ...
    if (!farmerContact) {
      Alert.alert('Error', 'Farmer contact information is required.');
      return false;
    }
    ...
    // Validate quantities
    for (const item of data.selectedItems) {
      const quantity = data.quantities[item];
    ...
    // Validate RMU selection if controller is selected
    if (controllerSelected) {
      if (data.withoutRMU === null) {
        Alert.alert('Error', 'Please select RMU or Without RMU option.');
        return false;
      }

      if (data.withoutRMU === true && !data.rmuRemark) {
          ...
          {selectedItems?.map((item, index) => (
            <View key={index}>
              ...
              <Controller
                ...
                name={`quantities.${item}`}
              />
              {errors.quantities?.[item] && (
                <Text style={styles.errorText}>
                  {errors.quantities[item].message}
                </Text>
              )}
            </View>
          ))}
