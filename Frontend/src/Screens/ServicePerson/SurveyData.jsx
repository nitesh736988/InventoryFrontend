...
const SurveyData = ({route}) => {
    ...
    launchCamera(
      {
        ...
        cameraType: 'back',
        quality: 1,
        // Remove includeBase64 since we're not using it
      },
      async response => {
        if (response.didCancel) {
          console.log('User cancelled camera picker');
          ...
          try {
            ...
            const resizedPhoto = {
              ...
              name: originalPhoto.fileName || `photo_${Date.now()}.jpg`,
            };
  ...
  const handleFormSubmit = async () => {
    ...
    try {
      ...
      formData.append('pumpHead', pumpHead);
      formData.append('category', category);
      formData.append('LTDistance', LTDistance);
      formData.append('remark', remark);

      // Add all image files
      fields.forEach(field => {
        images[field].forEach((photo, index) => {
          formData.append(field, {
            uri: photo.uri,
            type: photo.type || 'image/jpeg',
            name: photo.name || `${field}_${index}.jpg`,
          });
        });
      });
