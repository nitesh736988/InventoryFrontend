...
const LoginPage = () => {
  ...
  const [email, setEmail] = useState("");
  const [password, setPassword] = useState("");
  const [userType, setUserType] = useState(""); // State for the dropdown
  const [loading, setLoading] = useState(false);
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await axios.post('http://192.168.68.100:8080/user/login', { email, password, userType }, {timeout: 2000});
      if (response.status === 200) {
